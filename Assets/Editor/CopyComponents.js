...
@MenuItem ("MyMenu/Copy components")
static function Init () {
	Debug.Log("Component copier!");
...
function OnGUI () {
	...
	EditorGUILayout.Space();
	fromTrans = EditorGUILayout.ObjectField("From: ",fromTrans,Transform, true);
	toTrans = EditorGUILayout.ObjectField("To: ",toTrans,Transform, true);
	EditorGUILayout.Space();
...
function copyComponentsRecurse (src : Transform,  dst : Transform, originalSrc : Transform,  originalDst : Transform)
{
	...
	for (var srcComponent:Component in copyComps){
		if (srcComponent.GetType()==Animation&&anims){
			...
			new_animation.cullingType = helpsource.cullingType;
		}
